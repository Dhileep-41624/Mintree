...
public class PatientController {
    ...
    //     return patient;
    // }
    @GetMapping("/patients/patient/{name}")
    public Patient showPatientInfo(@PathVariable String name ){
        Patient patient = service.showPatientInfo(name);
        if(patient == null){
            throw new PatientNotFoundException(name + " is not in database");
        }
        return patient;
    }
